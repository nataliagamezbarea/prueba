...
# Configuración
directorio = r"C:\Users\Natalia\Desktop\pythons\1º GRADO MEDIO"
repositorio = "https://github.com/nataliagamezbarea/jkjkkj.git"
rama = "Primer_grado_medio"  # Cambié la rama a "Primer_grado_medio"
...
def inicializar_git(directorio):
    if not os.path.exists(os.path.join(directorio, ".git")):
        ...
        try:
            ...
            print("No se pudo sincronizar con el repositorio remoto.")
    else:
...
def agregar_y_subir_archivos(extension, directorio, repo, tamano_maximo_gb=2):
    ...
    if archivos_a_subir:
        print(f"Se subieron correctamente {len(archivos_a_subir)} archivos {extension} en Primer_grado_medio.")
    else:
...
def subir_lote(repo, lote, extension):
    try:
        print(f"Subiendo lote de {len(lote)} archivos {extension} en Primer_grado_medio...")
        for archivo in lote:
            repo.git.add(archivo)
        repo.git.commit(m=f"Subiendo lote de {len(lote)} archivos {extension} en Primer_grado_medio")
        ...
        for archivo in lote:
            try:
                repo.git.add(archivo)
                repo.git.commit(m=f"Subiendo archivo {os.path.basename(archivo)} en Primer_grado_medio")
                repo.git.push("--set-upstream", "origin", rama)
